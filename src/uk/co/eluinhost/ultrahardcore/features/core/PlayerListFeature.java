...
public class PlayerListFeature extends UHCFeature {
    ...
    private static final double HEALTH_SCALING = ConfigHandler.getConfig(ConfigHandler.MAIN).getDouble(ConfigNodes.PLAYER_LIST_SCALING);
    private static final boolean ROUND_HEALTH = ConfigHandler.getConfig(ConfigHandler.MAIN).getBoolean(ConfigNodes.PLAYER_LIST_ROUND_HEALTH);
    private static final boolean PLAYER_LIST_COLOURS = ConfigHandler.getConfig(ConfigHandler.MAIN).getBoolean(ConfigNodes.PLAYER_LIST_COLOURS);
    ...
    protected void enableCallback() {
        ...
        m_taskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(
                ...
                1L,
                ConfigHandler.getConfig(ConfigHandler.MAIN).getLong(ConfigNodes.PLAYER_LIST_DELAY)
        );
    ...
    private void initializeScoreboard() {
        ...
        m_objectiveUnderName.setDisplayName(ChatColor.translateAlternateColorCodes('&', ConfigHandler.getConfig(ConfigHandler.MAIN).getString(ConfigNodes.PLAYER_LIST_HEALTH_NAME)).replaceAll("&h", "\u2665"));
